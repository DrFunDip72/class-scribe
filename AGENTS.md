...
Owner-only Google Drive automation uses the Drive-for-desktop `URecorder` view with a read-only cloud fallback on the class schedule, links or queues recordings, gates transcript integrity, publishes summary-first notes to the four public course repositories, and audits weekly coverage. It accepts the observed standalone `Phil` alias and one-day-early recording labels. The importer runs in the owner's signed-in Windows session so it can access `G:`; the inference worker and weekly audit remain `SYSTEM` tasks. See `docs/DRIVE-GITHUB-AUTOMATION.md`.
...
\.venv-worker\Scripts\python.exe -m unittest -v test_class_scribe_automation.py

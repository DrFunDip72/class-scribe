...
export default async function JobPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!result) {
    return <main className="app-shell"><AppHeader email={user.email ?? "Account"} /><div className="result-shell"><Link className="back-link" href="/dashboard"><ArrowLeft size={15} /> Dashboard</Link><div className="processing-card"><Clock3 /><h1>{job.original_filename}</h1><p>{job.stage} · {tier.label} ({tier.model})</p><div className="progress-track"><span style={{ width: `${job.progress}%` }} /></div><Link className="button button-primary" href={`/jobs/${id}`}>Check again</Link></div></div></main>;
  }

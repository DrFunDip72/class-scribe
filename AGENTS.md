...
Owner-only Google Drive automation also reads `URecorder` on the class schedule, links or queues recordings, gates transcript integrity, publishes summary-first notes to the four public course repositories, and audits weekly coverage. See `docs/DRIVE-GITHUB-AUTOMATION.md`; the rclone shared OAuth client-ID replacement is required near-term.

...
Owner-only Google Drive automation reads the Drive-for-desktop `URecorder` view on the class schedule, links or queues recordings, gates transcript integrity, publishes summary-first notes to the four public course repositories, and audits weekly coverage. The importer runs in the owner's signed-in Windows session so it can access `G:`; the inference worker and weekly audit remain `SYSTEM` tasks. See `docs/DRIVE-GITHUB-AUTOMATION.md`.

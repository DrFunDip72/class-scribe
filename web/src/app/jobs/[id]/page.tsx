...
export default async function JobPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!result) {
    ...
    return <main className="app-shell"><AppHeader email={user.email ?? "Account"} /><div className="result-shell"><Link className="back-link" href="/dashboard"><ArrowLeft size={15} /> Dashboard</Link><div className="processing-card"><Clock3 /><h1>{job.original_filename}</h1><p>{currentStep} · {tier.label} quality</p><div className="progress-track"><span style={{ width: `${job.progress}%` }} /></div><Link className="button button-primary" href={`/jobs/${id}`}>Refresh status</Link></div></div></main>;
  }
  ...
  return <main className="app-shell"><AppHeader email={user.email ?? "Account"} /><article className="result-shell">
    ...
    <header className="result-header"><span className="result-file-icon"><FileAudio /></span><div><span className="section-kicker">Notes ready</span><h1>{job.original_filename}</h1><p><CheckCircle2 size={15} /> Ready {job.completed_at ? new Date(job.completed_at).toLocaleString() : "now"} · {tier.label} quality</p></div></header>
